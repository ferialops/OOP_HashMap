...
public class HashSet<V> {
    private EntrySet<V>[] entries;

    private int capacity;

    public HashSet(int capacity) {
        this.capacity = capacity;
        this.entries = new EntrySet[capacity];
    }
    ...
    public void put(V value) {
        ...
        EntrySet<V> newEntry = new EntrySet<>(value);
    ...
    public V get(V value) {
        ...
        int hashEntry = hash(new EntrySet<>(value));
    ...
    public int size() {
        ...
        for (EntrySet<V> bucket : Arrays.stream(entries).filter(Objects::nonNull).toList()) {
            var temp = bucket;
            while (temp != null) {
                count++;
                temp = temp.getNext();
    ...
    private int hash(EntrySet<V> entry) {
